...
?>
    <div class="minHeight">
        ...
        <div class="search">
            <span onclick="$('[name=WithdrawForm] [type=checkbox]').prop('checked',true);" class="spanButton">选择全部</span>
            <span onclick="$('[name=WithdrawForm] [type=checkbox]').prop('checked',false);" class="spanButton">取消选择</span>
            <span onclick="EditList('WithdrawForm','deleteWithdraw')" class="spanButton">删除所选</span>
            <span onclick="EditList('WithdrawForm','upateDraws')" class="spanButton">批量审核</span>
            <span class="smallWord floatRight">
			共找到<?php echo $num;?>条提现申请&nbsp;&nbsp;
            第<?php echo $page;?>页/
                ...
                if($num > 0){
                    while($adadWithdraw = mysql_fetch_array($query)){
                        ...
                        <tr>
                          ...
                          <td>".kong($kehuname['name'])."</td>
                          <td>{$adadWithdraw['moneny']}</td>
                          <td>{$adadWithdraw['workFlow']}</td>
                          <td>".kong($adname['adname'])."</td>
                          <td>{$adadWithdraw['updateTime']}</td>
                          <td>{$adadWithdraw['time']}</td>
                          <td><a href='{$root}control/adWithdrawMx.php?id={$adadWithdraw['id']}'><span class='spanButton'>审核</span></a></td>
                        </tr>
                        ";
                    }
                }else{
